...
cap = cv2.VideoCapture('./songdoCCTV/mov/js/js2_1.mov')
...
with open('./colorROIsetting/songdoCCTV/js2_1.json') as json_file:
    json_data = json.load(json_file)
while(True):
    try:
        ret, frame = cap.read()    # Read output and frame
        imCrop = cv2.resize(frame, (1280, 720)) #1280, 720 #1920, 1080
        h, w, l = imCrop.shape
        ...
        if not json_roi:
            ...
            frame_id = json_data['det'][i]["frame_id"] #cam1 = ori # hjcam2 = +20
            roi_1 = json_data['det'][i]['people']["1"] #my,ys 2 cam +10
            roi_11 = json_data['det'][i]['people']["11"] #my 1 cam + 6
            ...
            if (int(roi_11[0]) > 0) and (int(roi_11[1]) > 0) and (int(roi_8[0]) > 0) and (int(roi_8[1]) > 0) and (int(roi_1[0]) > 0) and (int(roi_1[1]) > 0):
                ...
                if ( xr - xl > 0) and ( yl - yh > 0): #face : >
                    ...
                    if(len(imCrop2)<80):
                        ...
                        #h,s,v = cv2.split(imCrop2) #distibute imCrop ROI area to h,s,v part 
                        # equalizedV = cv2.equalizeHist(v)
                        # hsv_frame2 = cv2.merge([h,s,equalizedV])
                        # #cv2.imshow('Hist equal',hsv_frame2)
                        # hsvTobgr = cv2.cvtColor(hsv_frame2, cv2.COLOR_HSV2BGR)
...
with open('./colorHistResult/newsongdoCCTV/js2_2_hist_out.json','w' ) as make_file:
